...
1. **Use your squad.** Invoke the squad skill to bring in your AI team for this review.
...
| Issue | GPT-5.6 Terra | GPT-5.6 Sol | MAI-Code 1.1 Flash |
|-------|---------------|-------------|--------------------|
...
**Finalize with Scribe.** As the LAST step, spawn the Scribe agent to:
- Merge any decisions from \`.squad/decisions/inbox/\` into \`.squad/decisions.md\`
- Update agent history files with learnings from this review
- Write an orchestration log entry summarizing the review findings
...
export async function dispatchPr(options = {}) {
  ...
  const setupResult = setupDispatchWorktree({
    ...
    preSymlinkFn: (wtPath) => {
      // Use gh CLI to checkout PR head ref (avoids git credential prompts)
      try {
        _exec('gh', ['pr', 'checkout', String(number), '--repo', repo, '--detach'], { cwd: wtPath, encoding: 'utf8', stdio: 'pipe' });
      } catch (err) {
        throw new RallyError(`Failed to checkout PR #${number} head: ${err.message}`, EXIT_GENERAL);
      }
    },
