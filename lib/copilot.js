...
export function getReadOnlyPolicy() {
  return [
    ...
    '## Model policy',
    '',
    '- When spawning sub-agents, explicitly use only these models:',
    '  - `gpt-5.6-terra` for architecture, security, and high-risk reasoning',
    '  - `gpt-5.6-sol` for implementation, testing, and code review',
    '  - `mai-code-1.1-flash` for bounded research and mechanical tasks',
    '- Never select any other model or omit the model parameter',
    '- If none of the allowed models is available, report the failure instead',
    '  of falling back to a model outside this allowlist',
    '',
...
export function launchCopilot(worktreePath, prompt, opts = {}) {
  ...
  // Check if squad agent exists in the worktree before using --agent squad
  const squadAgentPath = path.join(worktreePath, '.github', 'agents', 'squad.agent.md');
  const fsExists = opts._fs?.existsSync || existsSync;
  const hasSquadAgent = fsExists(squadAgentPath);

  if (!hasSquadAgent) {
    console.error('Warning: No squad agent found at .github/agents/squad.agent.md — running without squad agent');
  }
  ...
  try {
    ...
    if (logPath) {
      // Use restrictive permissions (owner read/write only) for log files
      // which may contain sensitive session data
      fd = fsOpen(logPath, 'w', 0o600);
      stdio = ['ignore', fd, fd];
    }

    const agentArgs = [
      // Only use --agent squad if the agent file exists in the worktree
      ...(hasSquadAgent ? ['--agent', 'squad'] : []),
      '--allow-all-tools',
      ...denyArgs,
...
export function parseSessionIdFromLog(logPath, opts = {}) {
  ...
  try {
    ...
    // Security: Only accept values that look like valid session IDs
    // (UUIDs or alphanumeric strings) to prevent command injection
    ...
    for (const pattern of legacyPatterns) {
      ...
      if (match) {
        ...
        // Validate: must be UUID or safe alphanumeric identifier (no shell metacharacters)
        if (UUID_RE.test(candidate) || /^[a-zA-Z0-9_-]{8,64}$/.test(candidate)) {
          return candidate;
        }
        // Reject suspicious values that could contain shell injection
      }
    }
  } catch {
...
export function resumeCopilot(worktreePath, sessionId, opts = {}) {
  ...
  const result = _spawnSync('copilot', args, {
    cwd: worktreePath,
    stdio: 'inherit',
  });
  ...
  if (result.error) {
    if (result.error.code === 'ENOENT') {
      throw new Error('Copilot CLI not found — install it and ensure "copilot" is on PATH');
    }
    throw result.error;
  }
