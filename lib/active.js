...
const VALID_STATUSES = ['implementing', 'reviewing', 'upstream'];
...
export function terminatePid(pid, _kill = process.kill.bind(process), _readFileSync = readFileSync) {
  ...
  // Verify the PID belongs to a Copilot CLI process before signaling
  try {
    const cmdline = _readFileSync(`/proc/${pid}/cmdline`, 'utf8');
    // cmdline uses NUL separators: "copilot\0..."
    const args = cmdline.split('\0');
    const executable = args[0] || '';
    if (!(executable.endsWith('/copilot') || executable === 'copilot')) return false;
  } catch {
...
export function addDispatch(record) {
  ...
  if (record.type !== 'issue' && record.type !== 'pr' && record.type !== 'branch') {
    throw new RallyError(`Invalid dispatch type: "${record.type}". Must be "issue", "pr", or "branch"`, EXIT_GENERAL);
  }
